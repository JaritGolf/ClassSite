/**
 * ParentSummaryView — presentational render of the allowlist parent VM
 * (Phase 14 / Phase 18). Shared by the teacher print view
 * (`/teacher/students/[id]/parent-summary`) and the parent dashboard
 * (`/parent/students/[id]`) so both surfaces show the identical, parent-safe
 * field set. Pure/presentational — no auth, no data fetching.
 */
...
export function ParentSummaryView({ summary }: { summary: ParentSummaryVM }) {
  return (
    <>
      ...
      {/* Current mission */}
      <section>
        <h2 className="text-sm font-semibold uppercase tracking-wide text-gray-600">Current Mission</h2>
        {summary.currentMission ? (
          <div className="mt-2 rounded-xl border border-indigo-100 bg-indigo-50/60 p-4">
            <p className="font-semibold text-gray-900">{summary.currentMission.title}</p>
            <p className="mt-1 text-sm text-indigo-700">{summary.currentMission.statusLabel}</p>
          </div>
        ) : (
          <p className="mt-2 text-sm text-gray-600">Ready to begin the next mission.</p>
        )}
      </section>
      ...
      <section className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className="rounded-xl border border-gray-200 bg-white p-4">
          <h2 className="text-sm font-semibold text-gray-700">
            Missions Mastered <span className="text-gray-600">({summary.mastery.mastered.length})</span>
          </h2>
          {summary.mastery.mastered.length === 0 ? (
            <p className="mt-2 text-sm text-gray-600">None mastered yet — that's okay!</p>
          ) : (
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-4">
          <h2 className="text-sm font-semibold text-gray-700">
            Needs Review <span className="text-gray-600">({summary.mastery.needsReview.length})</span>
          </h2>
          {summary.mastery.needsReview.length === 0 ? (
            <p className="mt-2 text-sm text-gray-600">Nothing flagged for review.</p>
          ) : (
            <ul className="mt-2 space-y-1 text-sm text-gray-700">
      ...
      <section>
        <h2 className="text-sm font-semibold uppercase tracking-wide text-gray-600">Recent Assessments</h2>
        {summary.recentAssessments.length === 0 ? (
          <p className="mt-2 text-sm text-gray-600">No assessments completed yet.</p>
        ) : (
            ...
            <tbody>
              {summary.recentAssessments.map((a, i) => (
                <tr key={i} className="border-b border-gray-50 last:border-0">
                  ...
                  <td className="py-2 text-right text-gray-600">{formatDate(a.date)}</td>
                </tr>
              ))}
            </tbody>
          </table>
